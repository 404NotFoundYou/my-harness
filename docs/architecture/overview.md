...
- `locking`：版本化所有者目录、原子发布/认领/释放及明确的失效锁诊断；损坏记录保持排他并显式失败。
...
- `guide`：只读任务引导，从工作项、计划和近期证据生成局部上下文及下一步参数；状态转换建议再经现有 validator 校验。业务判断、批准和审查保留为明确待办，不另建状态、权限或模型分档。
- `commands`：把验证声明编译为命令/参数数组，统一命令身份，并为 Windows 包管理器选择明确的 Node CLI 入口。
- `snapshot` / `verification`：持久化内容快照，按计划签名、任务尝试及修订验证命令证据；未运行、失效或新的失败不能用于完成声明。
- 可选验收映射保存在结构化检查中并参与计划签名；`guide.coverage` 暴露声明对应。旧无映射的检查与无附件的证据继续兼容。
- `scope`：区分活动范围、冻结交付和合法控制面；旧终态从实际提交归档。恢复设计见 [ADR-0007](decisions/0007-verification-integrity-and-recovery.md)。
...
6. 安装器生成带自校验哈希的安装收据，记录实际 Runtime/CI payload 与托管规则边界；卸载器必须从独立源仓库执行，只按收据移除归属可确定且源/目标内容均匹配的文件、有效托管块和初始化元数据。正式执行需要 `--confirm`，工作项、历史备份和项目文档始终保留，详见 [ADR-0003](decisions/0003-safe-uninstall.md)。
...
- 安装器无损合并根规则；普通文件冲突默认拒绝覆盖，强制替换必须使用显式选项并生成备份/变更报告。卸载器不提供强制删除冲突文件，在任何删除前完成备份，并在备份后及每项变更前复核目标快照。
