...
| 普通非琐碎工作 | 边界明确的功能迭代或 BUG，可涉及多个文件 | `begin → run → finish → check --ci`，一次填写方案，一次提交实际审查及验收结论 |
| 复杂或高风险工作 | 新项目、数据库、公共 API、权限、并发、重要架构或跨端变化 | 使用完整工作项、专项设计和适用的独立审查 |
| 持续分析 | 业务实现和技术方案调查 | ANALYSIS 工作项，产品文件只读 |

普通任务仍保留工作项和证据，由 Runtime 自动推进机械状态。已授权范围内的可逆实现由 AI 连续完成，不为普通技术选择反复确认；只有新增授权边界或关键歧义才询问。发现数据库、权限、支付、并发、全局 UI/路由等影响时升级完整流程，不拆分任务规避门禁。

精简入口只适用于单 AI、单纵向任务、low/medium 风险和无数据库影响。缺少成功命令、差异审查或验收结论时不会记为完成；原有细粒度命令用于复杂工作和中断恢复。具体参数见[普通任务默认路径](usage.md#普通任务默认路径)。
